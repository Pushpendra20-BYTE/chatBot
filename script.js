const API_KEY = "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"; // Ensure you set your API key
...
async function fetchGroqData(messages) {
  try {
    const response = await fetch(API_URL, {
      method: "POST",
      headers: {
        Authorization: `Bearer ${API_KEY}`, // Fix: Proper template literal
    ...
    if (!response.ok) {
      ...
      throw new Error(`HTTP error! Status: ${response.status}, Body: ${errorBody}`); // Fix: Proper template literal
    }
...
// Fix: Removed unnecessary appendMessage("") call
